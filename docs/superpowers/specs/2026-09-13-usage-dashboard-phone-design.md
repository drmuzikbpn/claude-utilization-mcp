...
Maintenance exit: long-press the top-left corner 3 s → PIN → lock task stops for 10 min.
Charge cap: if `/sys/class/power_supply/battery/charging_enabled` (or the LineageOS
equivalent) is writable, hold charge between 75–80 %; otherwise the setup doc says it is not
available on this device. Thermal: if battery temperature > 42 °C the app dims to 30 % and
drops sparkline redraws to every 10 s.
...
- Wifi screen: current SSID + RSSI + IP, scan results sorted by RSSI (dedup by SSID, show
  security type), saved networks with forget, "Open captive portal" launches a minimal
...
- Scan the QR printed by `claude-usage configure pairing`: `{ v, name, addr, port, token }`.
  Reject `v != 1`. Store as `MachineConfig` in `EncryptedSharedPreferences`.
...
- Events applied: `limits`, `spend` (today + delta → burn history), `session`
  (start/end/update), `pause` (rules + affected), `update`, `heartbeat`.
- **Fallback polling** when SSE has failed twice in a row: `GET /v1/summary` and
  `GET /v1/sessions` with `If-None-Match: W/"<rev>"` every 2 s (screen on) / 30 s (dimmed
  or off). Return to SSE on the next backoff tick.
- **Burn history:** per session and per project, a ring buffer of (timestamp, cumulative
  tokens) sampled from `session.update` and `spend.delta`; rate = tokens/min over a 60 s
  window; sparklines cover 30 min, project chart covers 5 h. Lost on app restart (v1).
...
- Error envelope `{ error: { code, message, hint } }` surfaces `hint` verbatim in a snackbar.
- All calls are `application/json`, no CORS, no cookies, timestamps parsed as ISO-8601 UTC.

## 8. Model (`model/`)

```
Machine { id, name, addr, port, token, user: User?, version, update: UpdateState, health: fresh|stale|dead }
User     { emailAddress, accountUuid, displayName }
Limits   { fiveHour: Window, sevenDay: Window, scoped: [Window] }   Window { percent, resetsAt, severity }
Project  { key: gitCommonDir ?: cwd, name, machineId, sessions: [Session] }
Session  { sessionId, pid?, alive, cwd, worktree?, model, startedAt, lastActivityAt,
           tokens {input, output, cacheCreate, cacheRead, messages}, pause: PauseState?, lastTool? }
PauseRule{ id, scope: all | project:<key> | session:<id>, mode: soft|hard, since, createdBy }
```
- Projects are grouped by `project.gitCommonDir` from the daemon, so worktrees of one repo
  roll up. Sessions the daemon back-filled without a pid show a "no pid" tag; hard pause is
  hidden for them (daemon returns 409 anyway).
- Team view derives from the union of machines: limits shown per user; spend summed.
...
- Scopes: session row → `session:<id>`; project header / drill-in → `project:<gitCommonDir>`;
  Pause-all bar → `all`. Sent as `POST /v1/pause { scope, mode, reason: "usage-deck" }`,
  resume as `POST /v1/resume { scope }`. `all` and project scopes are sent to **every**
  reachable machine that has matching sessions (project scope matches by `gitCommonDir`
  basename + name, since paths differ per machine; the drill-in shows which machines it hit).
- **Escalation:** a soft pause started from the phone carries a timer (default 90 s,
  configurable 30 s–10 min, or off). When it fires the app issues the hard request for the
  same scope. Rows show the countdown. Escalation is cancelled by resume or by the session
  ending. Soft pauses created elsewhere (CLI, other phone) do not escalate.
...
| unreachable | machine `dead` for 2 min | 2 short buzzes, once |
...
Header: name, worktree count, path. **Three tiles:** today tokens, tokens/min, share of the
owner's 5 h window (project tokens today ÷ user's tokens today × 5 h percent). Five-hour burn
chart (one y scale, gridlines, endpoint dot). In / out / cache % / messages row. Sessions
with short id, since, last tool call, escalation countdown, pause button. Bottom: Soft pause
all N, Hold · freeze.
...
- Every 10 min ±10 % (and on "check now"): `GET https://api.github.com/repos/<owner>/android-project/releases/latest`.
...
- Defer while a pause gesture is in progress or a hard-freeze hold is mid-press; otherwise
  install immediately. After install Android relaunches the launcher; the app restores the
...
Cellular; EAP wifi; light theme; more than one tailnet; historical spend beyond what the
...
§19, auto-update deferral state §20. Fixtures for sessions/pause/events to be published in
