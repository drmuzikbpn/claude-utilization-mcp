...
/**
 * Re-arms the kiosk after a maintenance exit.
 *
 ...
 * locked launcher, because `MainActivity` is the persistent HOME activity (spec §4).
 */
class LockTaskReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != ACTION_RELOCK) return
